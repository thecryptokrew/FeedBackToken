...
contract OrderStatisticTree {
    ...
    function update_count(uint value,uint type) private {
        Node n=nodes[type][value];
        n.count=1+nodes[n.children[false]].count+nodes[n.children[true]].count+n.dupes;
    }
    function update_counts(uint value,uint type) private {
        uint parent=nodes[type][value].parent;
        while (parent!=0) {
            update_count(parent);
            parent=nodes[parent].parent;
        }
    }
    function update_height(uint value,uint type) private {
        Node n=nodes[type][value];
        uint height_left=nodes[type][n.children[false]].height;
        uint height_right=nodes[type][n.children[true]].height;
        if (height_left>height_right)
            n.height=height_left+1;
        else
            n.height=height_right+1;
    }
    function balance_factor(uint value,uint type) constant private returns (int bf) {
        Node n=nodes[type][value];
        return int(nodes[n.children[false]].height)-int(nodes[n.children[true]].height);
    }
    function rotate(uint value,bool dir,uint type) private {
        bool other_dir=!dir;
        Node n=nodes[type][value];
        bool side=n.side;
        uint parent=n.parent;
        uint value_new=n.children[other_dir];
        Node n_new=nodes[type][value_new];
        uint orphan=n_new.children[dir];
        Node p=nodes[type][parent];
        Node o=nodes[type][orphan];
    ...
    function insert_helper(uint p_value,bool side,uint value) private {
        Node root=nodes[type][p_value];
        uint c_value=root.children[side];
        if (c_value==0){
            root.children[side]=value;
            Node child=nodes[type][value];
    ...
    function insert(uint value) isOwner() {
        ...
        if (value==0)
            nodes[type][value].dupes++;
        else{
            numberofInserts+=1;
            insert_helper(0,true,value);
        }
    }
    function rightmost_leaf(uint value,uint type) constant private returns (uint leaf) {
        uint child=nodes[type][value].children[true];
        if (child!=0)
            return rightmost_leaf(child);
        else
            return value;
    }
    function zero_out(uint value,uint type) private {
        Node n=nodes[type][value];
        n.parent=0;
        n.side=false;
        n.children[false]=0;
    ...
    function remove_branch(uint value,uint left,uint right,uint type) private {
        uint ipn=rightmost_leaf(left);
        Node i=nodes[type][ipn];
        uint dupes=i.dupes;
        remove_helper(ipn);
        Node n=nodes[type][value];
        uint parent=n.parent;
        ...
        if (left!=0) {
            ...
            nodes[type][left].parent=ipn;
        }
        if (right!=0) {
            i.children[true]=right;
            nodes[type][right].parent=ipn;
        }
        zero_out(value);
        update_counts(ipn);
    }
    ...
    function rank(uint value,uint type) constant returns (uint smaller){
       uint temp=0;
        if(value!=0){
            smaller=nodes[type][0].dupes;
            uint cur=nodes[type][0].children[true];
            Node cur_node=nodes[type][cur];
            while(temp<10){
              temp+=1;
                if (cur<=value){
                    if(cur<value)
                    ...
                    if (left_child!=0)
                        smaller+=nodes[type][left_child].count;
                }
                if (cur==value)
                    break;
    ...
    function select_at(uint pos) constant returns (uint value){
        uint zeroes=nodes[type][0].dupes;
        if (pos<zeroes)
            return 0;
        else {
    ...
    function duplicates(uint value) constant returns (uint n){
        return nodes[type][value].dupes+1;
    }
    ...
    function percentile(uint value) constant returns (uint k){
        uint pos=rank(value);
        uint same=nodes[type][value].dupes;
        uint num=count();
    ...
    mapping(uint=>mapping(uint => Node)) nodes;
}
